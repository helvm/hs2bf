...
library =
  ...
  where
    f op = [PushArg 2, PushArg 2, Arith op, Slide 3]
...
compileE mc mv (CrCase ec cs) = compileE mc mv ec |> Reduce RAny |> Case (map f cs)
  where
    f (con, vs, e) =
      ( M.findWithDefault (error $ "cE:not found:" ++ con) con mc,
        F.toList $
          ( UnPack (length vs)
...
pprintData (CrData name xs cons) =
  Group
    [ Line $ Span [Prim "data", Prim name],
      Indent $ Group $ zipWith cv cons ("=" : repeat "|")
    ]
  where
    cv (name, xs) eq = Line $ Span [U.Pack [Prim eq, Prim name], Prim $ show $ length xs]

pprintProc :: CrProc -> SBlock
pprintProc (CrProc n as e) =
  Group
...
pprintExprI (CrLet flag binds e) =
  ...
  where
    cv (v, e) = U.Pack [Prim v, Prim "=", pprintExprI e, Prim ";"]
...
-- | kind
data CrKind
  = -- | left associative application of types
...
data CrExpr
  ...
  | CrApp CrExpr CrExpr
  | -- | rec?
    CrLet Bool [(CrName, CrExpr)] CrExpr
  | CrCstr Int [CrExpr]
