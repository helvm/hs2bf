...
removeLoneSC m = M.filterWithKey (\k _ -> S.member k col) m
  where
    col = rlscAux m S.empty (S.singleton "main")
...
newFrame tag xs post =
  [ Comment $ unwords ["nf", show tag, show xs],
    ...
    Clear (Memory "Hp" 2),
    Val (Memory "Hp" 2) tag -- node tag
  ]
    ++ concatMap set (zip [3 ..] xs)
    ++ [ Clear (Memory "Hp" $ size - 1),
         Val (Memory "Hp" $ size - 1) size,
...
compileCode m (Push n : is) =
  contWith m StackT is $
    [ Inline "#stackNewS0" [],
      Copy (Memory "S0" $ negate $ n + 1) [Memory "S0" 0]
    ]
compileCode m (Slide n : is) =
  if n <= 0
    then error "Slide 0"
    else
      contWith m StackT is $
  ...
  contWith m StackT is $
    [  HS2BF.SAM.Alloc "x",
      ...
      ++ [ Delete "temp",
           ...
           Val (Memory "Hp" 1) 0,
           Clear (Memory "Hp" 2),
           Val (Memory "Hp" 2) $ tag op,
           Clear (Memory "Hp" 3),
           Move (Register "x") [Memory "Hp" 3],
           Delete "x",
           Clear (Memory "Hp" 4),
           Copy (Register "addr") [Memory "Hp" 4],
           Clear (Memory "Hp" 5),
           Val (Memory "Hp" 5) 6,
           Clear (Memory "Hp" 6),
           Inline "#heap1Hp" [],
...
pprintGMC (Case cs) =
  ...
  where
    f (label, xs) = Group [Line $ Span [Prim label, Prim "->"], Indent $ Group $ map pprintGMC xs]
