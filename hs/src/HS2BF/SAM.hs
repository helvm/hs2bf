...
allocateRegister (SAM rs [SProc name [] ss])
  ...
  where
    (rs', sss) = mapAccumL allocateRS [] ss
...
allocateRS rs (While p ss)
  ...
  where
    (rs', sss) = mapAccumL allocateRS rs ss
...
repackRS al rs d
  ...
  where
    allocate fr to = (fr, to, maybe undefined id $ lookup fr rs) : rs'
      where
        rs' = repackRS (S.insert to al) (filter ((/= fr) . fst) rs) d
...
data Stmt
  ...
  | -- | syntax sugar of 'Move'
    Copy Pointer [Pointer]
  | -- | syntax sugar of Move p []
    Clear Pointer
...
interpret = runProcessWithIO f . checkSAM "SAMi"
  where
    f (SAM rs procs) =
      let ptb0 = M.fromList $ map (procName &&& id) procs
...
dumpRegisterP proc (m0, m1) = unlines $ ("in " ++ proc ++ ":") : rs
  where
    rs =
      (map (\(n, v) -> "  " ++ n ++ ": " ++ showHex v "") $ M.assocs m0)
...
modifyPointer f = modify $ \x -> x {pointer = g $ pointer x}
  where
    g x = let y = f x in if x < 0 then error $ "modifyPointer: invalid pos: " ++ show y else y
